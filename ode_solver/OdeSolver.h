...
//=======================================================
// OdeSolver Class containing all methods needed to solve
...
class OdeSolver{
  private:
    ...
    // Number of x-points to store and number of equations
    int n, neq;
    ...
    // Storeage vectors
    std::vector<double> y, dydx, xstore;
  ...
  public:
    ...
    OdeSolver() : n(0), neq(0), is_ic_set(false) {}
    OdeSolver(int n, int neq, void (*rhs_func)(double, std::vector<double> &, std::vector<double> &)) : n(n), neq(neq), dydx_func(rhs_func),
         y(std::vector<double> (n,0.0)), dydx(std::vector<double> (n,0.0)), xstore(std::vector<double> (n,0.0)), 
         ystore(std::vector< std::vector<double> >(neq, std::vector<double>(n,0.0))), 
         hmin(0.0), eps(1e-8), h1(1e-4), nok(0), nbad(0), is_ic_set(false) {}
    ~OdeSolver(){}

    // Change the precision parameters
    void set_precision(double eps, double h1, double hmin){
      this->eps  = eps;
      this->hmin = hmin;
      this->h1   = h1;
    }
    ...
    // Return the solution (for variable i)
